...
class Identificacion extends CI_Controller {
	...
	public function index()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
				...
				$error=array('error'=>strtoupper("Falto informacion para poderse IDENTIFICAR. <br>".validation_errors()));
			...
			if(count($info)==0){
				...
				$error=array('error'=>"usuario o clave invalido");
				$this->load->view('contenido2',$error);
				...
				switch ($info['tipo']) {
					case 'D':
						# si es un DOCENTE
						redirect(base_url().index_page().'/docente');
						break;
					case 'E':
						# si es un Estudiante
						redirect(base_url().index_page().'/estudiante');
						break;
