...
public $dataSesion;
		/*
		* para verificar que este Logeado como Docente

		$this->dataSesion=$this->session->userdata('datos_usuario');
		if ($this->dataSesion['tipo']=='D') {}else{
	...
	public function enviar_correos_notificacion_noticia($data){
		...
		$this->email->message('<h3>'.$data['titulo'].'</h3><br><h4>Docente: '.$data['docente'].'</h4><br><p>  '.$data['descripcion'].' </p><hr><pre>Para visualizar el contenido de una mejor forma, puede acceder al portal en la seccion de noticias -> [ <a href="http://ktuxca.com/UPTA/index.php/noticias">http://ktuxca.com/UPTA</a>  ]</pre>');	
		if($this->email->send()){
