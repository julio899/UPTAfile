...
return $data_contenidos;
	...
	public function obtener_comentarios_contenido($id)
	{
			...
			$query=$this->db->query("SELECT `ktuxcaco_upta`.`cometarios_contenidos`.`id`,`ktuxcaco_upta`.`cometarios_contenidos`.`id_contenido`,`ktuxcaco_upta`.`cometarios_contenidos`.`fecha`,`ktuxcaco_upta`.`cometarios_contenidos`.`status`,`ktuxcaco_upta`.`cometarios_contenidos`.`comentario`,`ktuxcaco_upta`.`cuentas`.`usuario`,`ktuxcaco_upta`.`cuentas`.`nombre`,`ktuxcaco_upta`.`cuentas`.`apellido`,`ktuxcaco_upta`.`cuentas`.`email`,`ktuxcaco_upta`.`cuentas`.`tipo`
FROM  `ktuxcaco_upta`.`cometarios_contenidos` ,  `ktuxcaco_upta`.`cuentas` 
WHERE  `ktuxcaco_upta`.`cometarios_contenidos`.`id_contenido` =$id
AND  `ktuxcaco_upta`.`cuentas`.`idu` =  `ktuxcaco_upta`.`cometarios_contenidos`.`id_usuario` LIMIT 50");
		 	$comentarios=array();
	...
	public function insertar_comentario_contenido($data=''){
		//INSERT INTO `ktuxcaco_upta`.`cometarios_contenidos` (`id`, `id_contenido`, `id_usuario`, `comentario`, `fecha`, `status`) VALUES (NULL, '1', '1', 'algo de texto', NOW(), 'enviado');

		if($this->db->query("INSERT INTO `ktuxcaco_upta`.`cometarios_contenidos` (`id`, `id_contenido`, `id_usuario`, `comentario`, `fecha`, `status`) VALUES (NULL, '".$data['idc']."', '".$data['idu']."', '".$this->limpiarTodoEnCampo(mysql_real_escape_string($data['comentario']))."', NOW(), 'enviado')")){
			return 1;
		}else{
			return 0;
		}
...
public function actualizar_datos_docente($data=''){
	//UPDATE  `ktuxcaco_upta`.`cuentas` SET  `nombre` =  'Julio Cesarr',`apellido` =  'Vinachii',`email` =  'julio899@hotmail.com' WHERE  `cuentas`.`idu` =6;
		if($this->db->query("UPDATE  `ktuxcaco_upta`.`cuentas` SET  `nombre` =  '".$data['nombre']."',`apellido` =  '".$data['apellido']."',`email` =  '".$data['email']."' WHERE  `cuentas`.`idu` =".$data['idu'])){
			return 1;
		}else{
			return 0;
		}
...
public function actualizar_contenido_docente($data=''){
		if($this->db->query("UPDATE  `ktuxcaco_upta`.`contenidos` SET  `titulo` =  '".$data['data']['titulo']."',`contenido` =  '".$data['data']['contenido']."' WHERE  `contenidos`.`id` =".$data['data']['id_contenido'])){
			return 1;
		}else{
			return 0;
		}
...
public function insertar_noticia($datos){
		if($this->db->query("INSERT INTO `ktuxcaco_upta`.`noticias` ( `id` , `idu` , `titulo` , `descripcion` , `fecha` , `status` , `tipo` ) VALUES ( NULL , '".$datos['idu']."', '".$datos['titulo']."', '".$datos['descripcion']."', NOW( ) , 'A', 'N' )")){
			return 1;
		}else{
			return 0;
		}
...
public function insertar_contenido($datos){
	if($this->db->query("INSERT INTO  `ktuxcaco_upta`.`contenidos` ( `id` , `idu` , `titulo` , `contenido` , `fecha` , `nombre_archivo` , `status`) VALUES ( NULL,  '".$datos['idu']."',  '".$datos['titulo']."',  '".$datos['contenido']."', NOW( ) ,  '".$datos['archivo']."',  'A');")){
		return 1;
	}else{
		return 0;
	}
...
public function obtener_contenidos()
{
	$query=$this->db->query("SELECT * FROM  `contenidos` ORDER BY  `contenidos`.`fecha` DESC LIMIT 5");
		foreach ($query->result() as $columna ) 
		{
			$this->contenidos[]=array('id'=>$columna->id,'titulo'=>$columna->titulo,'contenido'=>$columna->contenido,'fecha'=>$columna->fecha);
		}
